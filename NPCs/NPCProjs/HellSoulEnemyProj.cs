...
namespace StormDiversMod.NPCs.NPCProjs
{
    public class HellSoulEnemyProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] <= homerandom)
                Projectile.rotation = (Main.LocalPlayer.Center - Projectile.Center).ToRotation() + MathHelper.ToRadians(270);
            else
                Projectile.rotation = (float)Math.Atan2((double)Projectile.velocity.Y, (double)Projectile.velocity.X) + MathHelper.ToRadians(270);
