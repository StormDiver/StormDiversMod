    ...
    public class DesertJarProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.GetModPlayer<EquipmentEffects>().frostJar == false) //Will change to frost if the player has the Frozen jar
            {
                ...
                if (Main.rand.Next(2) == 0)     //this defines how many dust to spawn
                {
                    int dust = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 10, Projectile.velocity.X * 1f, Projectile.velocity.Y * 1f, 130, default, 1.1f);
