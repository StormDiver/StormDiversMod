    ...
    public class CultistSentryProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                if (distance < 900f && !target.friendly && target.active && !target.dontTakeDamage && target.lifeMax > 5 && target.type != NPCID.TargetDummy)
                {
                    ...
                    if (Collision.CanHit(Projectile.Center, 0, 0, target.Center, 0, 0))
                    {
                        ...
                        if (shoottime > 25)
                        {
                            ...
                            for (int k = 0; k < 10; k++)
                            {
                                ...
                                int projID = Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), new Vector2(Projectile.Center.X, Projectile.Center.Y), new Vector2(perturbedSpeed.X, perturbedSpeed.Y), ModContent.ProjectileType<CultistSentryProj2>(), Projectile.damage, .5f, Projectile.owner, rotation.ToRotation(), ai);
		...
		public override void AI()
		{
			...
			if (Projectile.velocity == Vector2.Zero)
			{
				...
				if (Main.rand.Next(Projectile.extraUpdates) == 0 && (Projectile.velocity != Vector2.Zero || Main.rand.Next((Projectile.localAI[1] == 2f) ? 2 : 6) == 0))
				{
					...
					{
						Vector2 vector2 = Projectile.velocity.RotatedBy(1.5) * ((float)Main.rand.NextDouble() - 0.5f) * Projectile.width;
