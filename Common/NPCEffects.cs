...
namespace StormDiversMod.Common
{
    public class NPCEffects : GlobalNPC
    {
        ...
        public bool sludgeDebuff; //For sludge

        public bool sludgeVenomDebuff; //For Venom sludge
        ...
        public bool roseDOTdebuff; //Cresent Rifle Debuff drain
        ...
        public int flaskexplosivetime; //prevent Flask of Explosives from hitting triggered enemy

        public int stormimmunetime; //Prevent Storm Staff proj hitting same enemy
        ...
        public override void AI(NPC npc)
        {
            ...
            //slowdown enemies
            if ((beetled || ultrafrostDebuff || sludgeVenomDebuff) && (!npc.boss && NPCID.Sets.ShouldBeCountedAsBoss[npc.type] == false))
                npc.velocity.X *= 0.92f;
            else if ((webDebuff || BlizzardDebuff || sludgeDebuff) && (!npc.boss && NPCID.Sets.ShouldBeCountedAsBoss[npc.type] == false))
        ...
        {
            /*if (npc.townNPC) //makes all npcs fight
                npc.friendly = false;*/
        ...
        public override void UpdateLifeRegen(NPC npc, ref int damage)
        {
            if (glasssharddebuff)
            {
                ...
                // Count how many projs are attached to this npc.
                int shardCount = 0;
                foreach (var p in Main.ActiveProjectiles)
                {
                    if (p.type == ModContent.ProjectileType<GlassShardProj>() && p.ai[2] == 1f && p.ai[1] == npc.whoAmI) //ai 2 is the bool for it being stuck, ai 1 is for the target
                    {
                        shardCount++;
                    }
                }
            ...
            if (sludgeDebuff)
            {
                if (npc.buffImmune[BuffID.Poisoned] == false) //enemies immune to posion only suffer slowdown
                {
                    npc.lifeRegen -= 40;
                    damage = 4;
                }
            }
            if (sludgeVenomDebuff)
            {
                if (npc.buffImmune[BuffID.Venom] == false) //enemies immune to venom only suffer slowdown
                {
                    npc.lifeRegen -= 70;
                    damage = 7;
                }
            ...
            if (stungdebuff)
            {
                ...
                foreach (var p in Main.ActiveProjectiles)
                {
                    if (p.type == ModContent.ProjectileType<MossStingerProj>() && p.ai[0] == 1f && p.ai[1] == npc.whoAmI) //ai 0 is the bool for it being stuck, ai 1 is for the target
                    {
                        stingerCount++;
                    }
                }
                npc.lifeRegen -= stingerCount * 8; // 4 dps per stinger, 15 max, up to 60dps
                damage = stingerCount * 2;
                if (damage < stingerCount * 2)
                {
                    damage = stingerCount * 2;
                }
            }
        ...
        {  
            ...
            if (superFrost)
            {
                if (Main.rand.Next(4) < 3)
                {
                    int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 135, npc.velocity.X * 1.2f, npc.velocity.Y * 1.2f, 130, default, 2f); 
            ...
            if (superburnDebuff)
            {
                ...
                int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 6, npc.velocity.X * 1.2f, npc.velocity.Y * 1.2f, 0, default, 2f);
            ...
            if (hellSoulFire)
            {
                if (Main.rand.Next(4) < 3)
                {
                    int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 173, npc.velocity.X * 1.2f, npc.velocity.Y * 1.2f, 0, default, 2f); 
                    Main.dust[dust].noGravity = true; //this make so the dust has no gravity
                }
            }
            if (darknessDebuff)
            ...
            if (ultraburnDebuff)
            {
                int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 6, npc.velocity.X * 1.2f, npc.velocity.Y * 1.2f, 0, default, 2.5f);   
                Main.dust[dust].noGravity = true; //this make so the dust has no gravity
            ...
            if (ultrafrostDebuff)
            {
                int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 135, npc.velocity.X * 1.2f, npc.velocity.Y * 1.2f, 0, default, 2.5f); 
                Main.dust[dust].noGravity = true; //this make so the dust has no gravity
            ...
            if (spookedDebuff)
            {
                if (Main.rand.Next(5) == 0)
                {
                    int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 200, npc.velocity.X * 1.2f, -3, 0, default, 1f);
                    Main.dust[dust].noGravity = true; //this make so the dust has no gravity
            ...
            if (aridCoreDebuff)
            {
                if (Main.rand.Next(5) == 0)
                {
                    int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 138, npc.velocity.X * 1.2f, -3, 0, default, 1f); 
                    Main.dust[dust].noGravity = true; //this make so the dust has no gravity
                    Main.dust[dust].fadeIn = 0.5f; //this make so the dust has no gravity
            ...
            if (BlizzardDebuff)
            {
                int dust = Dust.NewDust(new Vector2(npc.position.X - 10, npc.position.Y - 10), npc.width + 20, npc.height + 20, 156, npc.velocity.X, npc.velocity.Y, 0, default, 1f);
                Main.dust[dust].noGravity = true; //this make so the dust has no gravity
            ...
            if (sludgeDebuff)
            {
                ...
                {
                    int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 39, npc.velocity.X, npc.velocity.Y, 150, default, 1f); 
                    Main.dust[dust].velocity *= 0; //this make so the dust has no gravity
                }
                drawColor = Color.OliveDrab;
            }
            if (sludgeVenomDebuff)
            {
                if (Main.rand.Next(4) == 0)
                {
                    int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 118, npc.velocity.X, npc.velocity.Y, 150, default, 1f);
                    Main.dust[dust].velocity *= 0; //this make so the dust has no gravity
                }
        ...
        public override void ModifyHitByProjectile(NPC npc, Projectile projectile, ref NPC.HitModifiers modifiers)
        {
            ...
            /*if (projectile.type == ModContent.ProjectileType<Projectiles.AmmoProjs.FrostBulletProj>()) //No crit from arid explosion
            {
            ...
            */
            ...
            if (projectile.GetGlobalProjectile<Common.SpinnerReflect>().reflected || projectile.type == ModContent.ProjectileType<SnowmanExplosion>()) //update damage to be the same as if it hit the player
            {
                if (Main.masterMode)
                {
        ...
        public override void GetChat(NPC npc, ref string chat)
        {
            ...
            if (npc.type== NPCID.PartyGirl && Main.bloodMoon)
            {
            ...
            if (Main.LocalPlayer.armor[0].type == ModContent.ItemType<zAprilFools.TheGoldenPainMask>() || Main.LocalPlayer.armor[10].type == ModContent.ItemType<zAprilFools.TheGoldenPainMask>())
            {
                if (Main.rand.Next(4) == 0)
                {
                    switch (Main.rand.Next(3))
