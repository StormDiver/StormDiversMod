    ...
    public class TwilightPetBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
            {
                ...
                Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), new Vector2(player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2)), new Vector2(0, 0), ProjectileType<TwilightPetProj>(), 0, 0, player.whoAmI);
